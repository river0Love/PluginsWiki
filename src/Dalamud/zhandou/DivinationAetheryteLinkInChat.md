---
title: 快速传送Divination.AetheryteLinkInChat
icon: /assets/icon/AetheryteLinkInChat.png
order: 1
category:
  - 战斗
tag:
  - 传送
---

## 插件信息

![Divination.AetheryteLinkInChat =100x](/assets/icon/AetheryteLinkInChat.png)

插件名称：Divination.AetheryteLinkInChat
简述：快速传送以进行怪物狩猎和寻宝！
作者：Asteria Nocturna

## 介绍

Divination.AetheryteLinkInChat 从游戏聊天信息的地图链接，中显示一个传送链接到最近的以太之光的链接。输入 /alic 来配置插件。

## 裤链

::: tabs

@tab 中文
https://raw.githubusercontent.com/zhouhuichen741/dalamud-plugins/master/repo.json
https://raw.githubusercontent.com/NiGuangOwO/DalamudPlugins/main/pluginmaster.json
https://raw.githubusercontent.com/RedAsteroid/DalamudPlugins/main/pluginmaster.json

@tab 英文
https://github.com/SlashNephy/divination-distrib/raw/master/plugins/master.json

:::

## 使用方法