---
title: 物品价格查看Price Insight CN
icon: /assets/icon/PriceInsight-CN.png
order: 1
category:
  - 工具
tag:
  - marketboard
  - items
  - price
---

## 插件信息

![Price Insight CN =100x](/assets/icon/PriceInsight-CN.png)

插件名称：Price Insight CN
简述：将光标悬停在物品上来查看市场价格
作者：Kouzukii
项目地址：https://github.com/Ookura-Risona/ffxiv-priceinsight

## 介绍

当您鼠标悬停在物品上时会在物品信息底部展示该物品的 NQ/HQ 市场价格以及最近一次交易记录。
价格从 Universalis.app 中搜索，缓存保存90分钟。
您可以将鼠标悬停在物品上，点击Alt来刷新价格。

## 裤链

::: tabs

@tab 中文
原始链接（推荐）：https://raw.githubusercontent.com/Ookura-Risona/DalamudPlugins/main/pluginmaster.json
国内加速：https://gh.risona.top/pluginmaster.json

https://raw.githubusercontent.com/Ookura-Risona/DalamudPlugins/main/pluginmaster.json

@tab 英文
https://raw.githubusercontent.com/Dalamud-DailyRoutines/PluginDistD17/refs/heads/main/pluginmaster.json

:::

## 使用方法